        ...
        var methods = {
            init: function(options) {
                ...
                var mdp_events = {
                    ...
                    beforeShowDay: function(date) {
                        ...
                        //var highlight_class = gotThisDate ? '' : custom[1];
                    ...
                    onClose: function(dateText, inst) {
                        ...
                        if (this.multiDatesPicker.originalOnClose)
                            this.multiDatesPicker.originalOnClose.call(this, dateText, inst);
                    }
                };
                ...
                if (altFieldOption)
                    $(altFieldOption).val($this.multiDatesPicker('getDates', 'string'));
            },
            ...
            addDates: function(dates, type) {
                if (dates.length > 0) {
                    ...
                    var dtsTime = Date.parse(dates);
                    var arr = new Array();
                    var c = 0;
                    $('#altField li').each(function() {
                        arr[c] = parseInt($(this).attr('dst'));
                        c++;
                    });
                    $('#chonse').show();
                    if(!$(".fdts-"+ dtsTime).hasClass( "ui-state-red" )){
                        arr[arr.length + 1] = dtsTime;
                    }else{
                        alert("Ngày này đã hết vị trí quảng cáo, Xin vui lòng chọn ngày khác");
                        return false;
                    }                    
                    var arrSort = arr.sort();
                    var htmlDate = '';
                    var x = 0;
                    for (var dt = 0; dt < arrSort.length; dt++) {
                        if (!isNaN(arrSort[dt]) && typeof arrSort[dt] !== 'undefined') {
                            x++;
                            var dd = new Date(arrSort[dt]);
                            htmlDate += '<li dst="' + arrSort[dt] + '">' + dd.getDate() + '/' + (dd.getMonth() + 1) + '/' + dd.getFullYear() + ' (<a href="javascript:void(0);" onclick="vip.remove(' + arrSort[dt] + ');">x</a>)</li>';
                        }
                    }
                    $('#xeng').html(x * 50000);
                    $('#altField').html(htmlDate);
                    // $('#altField').append('<li dst="' + dtsTime + '">' + dd.getDate() + '/' + (dd.getMonth() + 1) + '/' + dd.getFullYear() + ' (<a href="javascript:void(0);" onclick="vip.remove(' + dtsTime + ');">x</a>)</li>');
            ...
            removeDates: function(dates, type) {
                ...
                //dungnv3 fuck date
                var dtsTime = Date.parse(dates);
                var dd = new Date(dtsTime);
                $('#altField li').each(function() {
                    if (parseInt($(this).attr('dst')) === dtsTime) {
                        $(this).remove();
                    }
                });
                var c = 0;
